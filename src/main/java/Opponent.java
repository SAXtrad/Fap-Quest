...
public class Opponent {
	...
	public void updateHUD(){
		...
		for(int i=0;i<opponent_attacks.length && !avatar;i++){ 
			avatar = opponent_attacks[i].toLowerCase().contains("avatar");
		}
		Image img_avatar = null;
		if(avatar){
			img_avatar = new ImageIcon("opponents/"+name+"/avatar.png").getImage();
		}else{
	...
	public void loadSilhouetteNextOpponent(int x_beat){
		...
		for(int i=0;i<opponent_attacks.length && !silhouette;i++){
			silhouette = opponent_attacks[i].toLowerCase().contains("silhouette");
		}
		...
		for(int i=0;i<opponent_attacks.length;i++){
			if(!(opponent_attacks[i].toLowerCase().matches("introduction") || opponent_attacks[i].toLowerCase().matches("end") || opponent_attacks[i].toLowerCase().contains("silhouette") || opponent_attacks[i].toLowerCase().contains("avatar"))){
				File opponent_attack_repertory = new File("opponents/"+name+"/"+opponent_attacks[i]);
		...
		if(silhouette){
			img_silhouette = new ImageIcon("opponents/"+name+"/silhouette.png").getImage();
		}else{
	...
	public String getNameImgIntroduction(){
		...
		/*for(int i=0;i<introduction_imgs.length;i++){ 
			System.out.println(introduction_imgs[i]); 
		}*/
	...
	/*public void loadNewAttack() {
		if(attacks.isEmpty()){
			...
			for(int i=0;i<opponent_attacks.length;i++){ 
				if(!(opponent_attacks[i].toLowerCase().matches("introduction") || opponent_attacks[i].toLowerCase().matches("end") || opponent_attacks[i].toLowerCase().contains("silhouette") || opponent_attacks[i].toLowerCase().contains("avatar"))){
					attacks.add(opponent_attacks[i]);
				}
			}
		}

		int random_attack = (int) (Math.random() * attacks.size());
		current_attack = attacks.get(random_attack);
